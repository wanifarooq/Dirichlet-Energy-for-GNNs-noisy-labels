...
To operationalize spectral bias in GNNs, we evaluate both GCN~\cite{gcn} and GIN~\cite{WL1} layer update mechanisms. For each input graph $\mathcal{G}_i$, we define the GCN update rule as:
...
\subsubsection{Ablation: Training Accuracy Feedback}

The incorporation of $a_{\text{train}}$ in Equations~\ref{eq:gcod1} and~\ref{eq:gcod3} is critical. Without $a_{\text{train}}$ modulation, $\mathbf{u}$ dominates early in training, prematurely discounting samples before meaningful patterns are learned. With $a_{\text{train}}$, discounting activates gradually, yielding 3--5\% higher test accuracy under 20\% noise.
