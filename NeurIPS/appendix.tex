...
\begin{equation}
h_\theta(\mathbf{X})=\sigma_2\bigl(\mathbf{W}_2\,\sigma_1(\mathbf{W}_1\,\mathbf{M}\mathbf{X})\bigr),
\end{equation}
acting on $\mathbf{X}\in\mathbb{R}^{N\times m}$ with $\mathbf{M}\in\mathbb{R}^{N\times N}$ and $\mathbf{W}_k$ acting on the feature dimension. For leaky-ReLU activation (or ReLU with a fixed margin $\delta>0$ along $\gamma(t)=\mathbf{X}+t\Delta\mathbf{X}$, ensuring no piecewise-linear region change occurs), $\sigma_k$ has derivative $\geq\alpha$ everywhere along the path. Differentiating w.r.t. $\mathbf{X}$ in the row-by-row sense (each row of $\mathbf{X}$ propagates through the same MLP, with rows mixed only by $\mathbf{M}$),
\begin{equation}
\frac{\partial h_\theta}{\partial \mathbf{X}}=\mathbf{D}_2\,\mathbf{W}_2\,\mathbf{D}_1\,\mathbf{W}_1\,\mathbf{M},
\end{equation}
where $\mathbf{D}_k=\mathrm{diag}(\sigma_k')$ are diagonal with entries in $[\alpha,1]$. By submultiplicativity of the smallest singular value for full-rank factors (each factor here is square or has full column rank by hypothesis on $\sigma_{\min}(\mathbf{W}_k),\sigma_{\min}(\mathbf{M})>0$),
\begin{align}
\sigma_{\min}(\mathbf{J}(\mathbf{X})) &\geq \sigma_{\min}(\mathbf{D}_2)\,\sigma_{\min}(\mathbf{W}_2)\,\sigma_{\min}(\mathbf{D}_1)\,\sigma_{\min}(\mathbf{W}_1)\,\sigma_{\min}(\mathbf{M})\nonumber\\
&\geq \alpha\cdot m_2\cdot \alpha\cdot m_1\cdot \kappa = \alpha^2 m_1 m_2 \kappa = c_{\text{lower}}.
\end{align}
For any $\mathbf{X},\Delta\mathbf{X}$, set $\gamma(t)=\mathbf{X}+t\,\Delta\mathbf{X}$. Under the activation regularity above, $h_\theta\circ\gamma$ is a piecewise-linear curve with no region change; the fundamental theorem of calculus then gives
\begin{equation}
h_\theta(\mathbf{X}+\Delta\mathbf{X})-h_\theta(\mathbf{X})=\int_0^1\mathbf{J}(\gamma(t))\,\Delta\mathbf{X}\,dt.
\end{equation}
Taking Frobenius norms and applying $\|\mathbf{J}\,\Delta\mathbf{X}\|_F\geq\sigma_{\min}(\mathbf{J})\,\|\Delta\mathbf{X}\|_F$ pointwise yields
\begin{equation}
\|h_\theta(\mathbf{X}+\Delta\mathbf{X})-h_\theta(\mathbf{X})\|_F\geq c_{\text{lower}}\,\|\Delta\mathbf{X}\|_F.
\end{equation}
For a true two-layer GIN $\sigma_2(\mathbf{W}_2\,\mathbf{M}\,\sigma_1(\mathbf{W}_1\,\mathbf{M}\mathbf{X}))$, an additional factor of $\sigma_{\min}(\mathbf{M})\geq\kappa$ enters the chain, giving $c_{\text{lower}}=\alpha^2 m_1 m_2 \kappa^2$; the rest of the argument is identical. $\square$
...
For any threshold $\tau$, define $n^i(\tau)=|\{j:\lambda^i_j\geq\tau\}|$. We claim $n^i(\tau)$ is non-decreasing under (a) edge addition and (b) vertex addition at fixed sparsity. \emph{(a) Edge addition}: if $\mathcal{G}^{i\prime}=\mathcal{G}^i+e$ is obtained by adding edge $e=(u,v)$, then $\mathbf{L}^{i\prime}=\mathbf{L}^i+\mathbf{e}_{uv}\mathbf{e}_{uv}^\top$, a rank-one positive update where $\mathbf{e}_{uv}=\mathbf{e}_u-\mathbf{e}_v$. By Weyl's interlacing theorem, the eigenvalues of $\mathbf{L}^{i\prime}$ majorize those of $\mathbf{L}^i$ pointwise, so $n^i(\tau)$ does not decrease. \emph{(b) Vertex addition}: adjoining a new vertex with at least one edge enlarges the matrix to $\mathbf{L}^{i\prime}\succeq\mathbf{L}^i\oplus[0]$, again leaving the count $n^i(\tau)$ non-decreasing by Cauchy interlacing for principal submatrices.
