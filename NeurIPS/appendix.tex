...
\textbf{Fundamental theorem of calculus.} For $\gamma(t)=\mathbf{X}+t\,\Delta\mathbf{X}$, under the activation regularity above $h_\theta\circ\gamma$ has the same piecewise-linear region throughout, so
\begin{equation}
\mathrm{vec}\bigl(h_\theta(\mathbf{X}+\Delta\mathbf{X})-h_\theta(\mathbf{X})\bigr)=\Bigl(\textstyle\int_0^1\mathbf{J}(\gamma(t))\,dt\Bigr)\,\mathrm{vec}(\Delta\mathbf{X}).
\end{equation}
Taking the $\ell_2$ norm of both sides (= Frobenius norm in matrix form) and using $\|\mathbf{J}\,\mathbf{x}\|_2\geq\sigma_{\min}(\mathbf{J})\,\|\mathbf{x}\|_2$ pointwise (so $\|\int\mathbf{J}\,dt\,\mathbf{x}\|\geq c_{\text{lower}}\|\mathbf{x}\|$ by Jensen on the singular-value lower bound),
\begin{equation}
